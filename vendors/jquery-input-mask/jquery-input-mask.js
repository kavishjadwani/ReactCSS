/**
 * Copyright (c) 2010, Bonafide, Inc. All rights reserved.
	...
	if (v.length>0)
	{
		if( d.getDate() != dd ) 
		{
			if (this.vd) {
				...
				alert("Please enter a correct date in the format of mm/dd/yyyy.");
				if (window.event) window.event.srcElement.focus();
	...
	for( i=0; i < a.length; i++ ){
		...
		if( x == "m" ){
			...
			if( a[i].length == 2 ) {
				...
				if (_yyyy > 0) {
					if (_yyyy > 999) nv += (new String(_yyyy)).substring(2);
					else if (_yyyy > 99) nv += (new String(_yyyy)).substring(1);
					else if (_yyyy > 9) nv += new String(_yyyy);
					else nv += '0' + _yyyy;
				}
				else {
					nv += d.getYear();
...
/*
 * Global utility mask objects declaration.
 */
var oDateMask = new Mask('mm/dd/yyyy', 'date');
var oAnyDateMask = new Mask('mm/dd/yyyy', 'date', true);
var oDollarMask = new Mask('#.00', 'number');
var oNumberMask = new Mask('#', 'number');
var oPhoneMask = new Mask('(###) ###-####');
var oSimpleDateMask = new Mask('mm/dd/yy', 'date');
var oSocialSecurityNumberMask = new Mask('###-##-####');
var oTripleMask = new Mask('#.000', 'number');
var oQuadrupleMask = new Mask('#.0000', 'number');

/**
 * Plugin definition for masks.
 * @param type
 */
(function($) {
	jQuery.fn.mask = function(type) {
		return this.each(function() {
			if (type) {
				if (type == MaskType.DATE) {
					oDateMask.attach(this);
				}
				else if (type == MaskType.ANYDATE) {
					oAnyDateMask.attach(this);
				}
				else if (type == MaskType.DOLLAR) {
					oDollarMask.attach(this);
				}
				else if (type == MaskType.NUMBER) {
					oNumberMask.attach(this);
				}
				else if (type == MaskType.PHONE) {
					oPhoneMask.attach(this);
				}
				else if (type == MaskType.SIMPLEDATE) {
					oSimpleDateMask.attach(this);
				}
				else if (type == MaskType.SSN) {
					oSocialSecurityNumberMask.attach(this);
				}
				else if (type == MaskType.TRIPLE) {
					oTripleMask.attach(this);
				}
				else if (type == MaskType.QUADRUPLE) {
					oQuadrupleMask.attach(this);
				}
			}
		});
	};
